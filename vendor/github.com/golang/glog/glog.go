...
const (
	debugLog severity = iota //add debugLog by chendonghe20190807
	infoLog
	warningLog
	errorLog
	fatalLog
...
func (l *loggingT) output(s severity, buf *buffer, file string, line int, alsoToStderr bool) {
	...
	if !flag.Parsed() {
		...
		if alsoToStderr || l.alsoToStderr {
			fmt.Printf("alsoToStderr:%+v\n", alsoToStderr)
			fmt.Printf("l.alsoToStderr:%+v\n", l.alsoToStderr)
			os.Stderr.Write(data)
		}
		if l.file[s] == nil {
...
func (sb *syncBuffer) rotateFile(now time.Time) error {
	...
	var err error
	sb.file, _, err = create(severityName[sb.sev], firstInvoke)
	buf_len, _ := sb.file.Seek(0, os.SEEK_END)
	sb.nbytes = uint64(buf_len)
	if err != nil {
		return err
	}
	...
	fmt.Fprintf(&buf, "\n\n----------------------------------\n")
	fmt.Fprintf(&buf, "Log Content created at: %s\n", now.Format("2006/01/02 15:04:05"))
